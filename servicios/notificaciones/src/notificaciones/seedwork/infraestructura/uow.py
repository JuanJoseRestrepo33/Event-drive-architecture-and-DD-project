"""Unidad de Trabajo (UoW): coordina los batches de operaciones sobre
repositorios y PUBLICA los eventos del agregado como señales pydispatch:
- señal `<Evento>Dominio` al registrar el batch (comunicación ENTRE MÓDULOS
  por eventos de dominio: pagos se conecta a `CotizacionAceptadaDominio`);
- señal `<Evento>Integracion` después del commit (los handlers de aplicación
  despachan el evento de integración al broker).
Alineado con el seedwork del tutorial del curso."""
    ...
    def _publicar_eventos_dominio(self, batch, repositorio_eventos_func):
        for evento in self._obtener_eventos(batches=[batch]):
            if repositorio_eventos_func:
                ...
                print(f'[uow] evento {type(evento).__name__} escrito en el EVENT STORE')
            print(f"[uow] señal '{type(evento).__name__}Dominio' emitida (pre-commit, pydispatch)")
            dispatcher.send(signal=f'{type(evento).__name__}Dominio', evento=evento)

    def _publicar_eventos_post_commit(self):
        try:
            for evento in self._obtener_eventos():
                print(f"[uow] señal '{type(evento).__name__}Integracion' emitida (post-commit)")
...
def flask_uow():
    from flask import session
    from notificaciones.config.uow import UnidadTrabajoSQLAlchemy
    if session.get('uow'):
        return session['uow']
...
class UnidadTrabajoPuerto:
    ...
    def registrar_batch(operacion, *args, lock=Lock.PESIMISTA, repositorio_eventos_func=None, **kwargs):
        uow = unidad_de_trabajo()
        uow.registrar_batch(operacion, *args, lock=lock,
                            repositorio_eventos_func=repositorio_eventos_func, **kwargs)
        guardar_unidad_trabajo(uow)
